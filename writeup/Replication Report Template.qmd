...
- Pilot A: [QSF file](REPLACE WITH LINK), [Survey link](REPLACE WITH LINK), [Data collected](REPLACE WITH LINK)
...
data.url <- "https://raw.githubusercontent.com/psych251/child2018/master/data/psych251_child2018_anonymized_experiment.csv" #REPLACE WITH LINK
charCount.url <- "https://raw.githubusercontent.com/psych251/child2018/master/data/Child_Oakhill_Garnham_2018_Sentence_CharacterCount.csv" #REPLACE WITH LINK
...
filtered_data = filtered_data %>%
  dplyr::mutate_at(vars(ends_with('RT') | ends_with('ER')), as.numeric) %>%
...
filtered_data = filtered_data %>%
  ...
  sum(!is.na(filtered_data %>% dplyr::select(ends_with('RT')))) 
    # new no. of datapoints = 4926 (i.e., 152 datapoints removed)
...
reg_RT_data = lmer(log_RT ~ 1 + Character_Count + (1 | Subject), long_RT_data)
