...
[not done] Original effect size, power analysis for samples to achieve 80%, 90%, 95% power to detect that effect size.  Considerations of feasibility for selecting planned sample size.
...
In line with the original experiment, participants were presented with 48 text passages (24 unique passages, each with two versions: one written in a personal, second person perspective and the other written in an onlookers', third person perspective). The passages were obtained from the [supplemental materials](https://github.com/psych251/child2018_rescue/blob/main/original_paper/supplemental_materials.pdf) uploaded by the authors of the original study. Each passage was 5-9 sentences long and each passage ended with an explicit emotion word that matched the content of the passage.  Passages differed by both emotional valence (negative versus positive) and perspective (personal versus onlooker). Thus, of the 48 passages:
- 12 were negatively-valenced, written in an personal (second-person) perspective;
- 12 were positively-valenced, written in an personal (second-person) perspective;
- 12 were negatively-valenced, written in an onlooker's (third-person) perspective;
- and 12 were positively-valenced, written in an onlooker's (third-person) perspective.

The passages were counterbalanced by perspective, valence and gender of the character for all participants. 
...
The main result that the present study seeks to replicate is the finding that "reading times were faster for passages that were written from a personal perspective than from an onlooker perspective" (Child et al., 2018; p.883). To test replicate this finding, a pairwise comparison will be conducted to examine if within the positively-valenced passages, reading times differed by the perspective the passages were written in.
...
#### Load Relevant Libraries and Functions
  ...
  # load psych for descriptives
  # load lme4 and lmrTest for LME

#### Import data

#### Data exclusion / filtering
  # Exclude participants who spend less than 400ms on more than half of trials
  # Exclude reading times that are longer than 15 seconds
  # Exclude reading times that are more than 2.5sd from each individuals' mean

#### Prepare data for analysis - create columns etc.
  # Convert data into long format
  # Create column indicating unique trial numbers (for random intercept of item)
  # Compute log-residual reading times
...
```{r include=T}
### LME code here
# also add three panel graph here
```

To test if participants read positively-valenced passages faster when they were written from a personal (second person's) perspective compared to when they were written from an onlooker's (third person's) perspective, pairwise comparisons were conducted. Results show that ...
